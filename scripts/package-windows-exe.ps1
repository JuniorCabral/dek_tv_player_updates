...
#   .\scripts\package-windows-exe.ps1 [-ReleaseDir caminho] [-OutExe caminho] [-Version 1.0.8]
...
if (-not (Test-Path $7za)) { throw "7za nao encontrado em $7za — rode o bootstrap de tools uma vez." }
...
RunProgram="app_player.exe"
;!@InstallEnd@!
"@ | Set-Content -Path $configPath -Encoding Ascii

$archive = Join-Path $work "payload.7z"
if (Test-Path $archive) { Remove-Item $archive -Force }
& $7za a -t7z -mx=7 $archive "$ReleaseDir\*"
